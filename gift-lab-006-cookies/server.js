...
app.use(cookieParser('bugforge_gift_lab_006_2026'));
...
async function initDb() {
  ...
  dbRun(`INSERT INTO users (username, password_hash, role) VALUES (?, ?, ?)`, ["administrator", administratorPass, "administrator"]);
  // Seed lists
...
app.get('/dashboard', requireLogin, (req, res) => {
  ...
  const lists = dbAll(`SELECT * FROM lists WHERE user_id = ? ORDER BY id DESC`, [req.signedCookies.userId]);
...
app.get('/list/:id', requireLogin, (req, res) => {
  ...
  const list = dbGet(`SELECT * FROM lists WHERE id = ? and user_id = ?`, [listId, req.signedCookies.userId]);
  if (!list) {
...
app.post('/lists', requireLogin, (req, res) => {
  ...
  dbRun(`INSERT INTO lists (user_id, title) VALUES (?, ?)`, [req.signedCookies.userId, newList]);
  res.redirect('/dashboard');
});
...
app.post('/lists/:id/delete', requireLogin, (req, res) => {
  const listId = req.params.id;
  const list = dbGet('SELECT id FROM lists WHERE id = ? AND user_id = ?', [listId, req.signedCookies.userId]);
  if (!list) {
...
app.post('/lists/:id/share', requireLogin, (req, res) => {
  ...
  dbRun(`UPDATE lists SET share_token = ? WHERE id = ? AND user_id = ?`, [token, listId, req.signedCookies.userId]);
...
app.post('/delete/:item_id/:list_id', requireLogin, (req, res) => {
  ...
  const list = dbGet(`SELECT id FROM lists WHERE id = ? AND user_id = ?`, [list_id, req.signedCookies.userId]);
  if (!list) {
    return res.status(403).send("Nice try. That's not your list.");
  }
...
// Admin
const ADMIN_FLAG = "bug{brut3f0rce_cookie}";

app.get("/administrator", requireLogin, (req, res) => {
  if(req.cookies.session === "YnVnZm9yZ2Utcmxz"){
